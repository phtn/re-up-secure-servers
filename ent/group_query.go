...
func (gq *GroupQuery) QueryAccount() *AccountQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, group.AccountTable, group.AccountColumn),
		)
		fromU = sqlgraph.SetNeighbors(gq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAccount: gq.withAccount.Clone(),
...
func (gq *GroupQuery) WithAccount(opts ...func(*AccountQuery)) *GroupQuery {
	query := (&AccountClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withAccount = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withAccount; query != nil {
		if err := gq.loadAccount(ctx, query, nodes, nil,
			func(n *Group, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Group, init func(*Group), assign func(*Group, *Account)) error {
...
func (gq *GroupQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := gq.ctx.Fields; len(fields) > 0 {
		...
		if gq.withAccount != nil {
			_spec.Node.AddColumnOnce(group.FieldAccountID)
		}
	}
